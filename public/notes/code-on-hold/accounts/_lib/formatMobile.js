...
export function formatMobile(mobile) {
  ...
  if ([...mobile].some(char => char.match(/[a-z]/i))) return null;

  // build right to left
  // mobile="+920321-8820892" => result="8820892" 
  let result = mobile.slice(-7);
  if ([...result].some(c => !Number.isInteger(parseInt(c)))) return null;
  ...
  if (mobile.charAt(mobile.length - 10) !== "3") return null;
