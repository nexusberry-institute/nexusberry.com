...
import type { Department, HomePage } from '@/payload-types'

type TopDepartmentsProps = {
  config?: HomePage['departmentsSection']
  departments: Pick<Department, 'title' | 'slug' | 'image'>[]
}

export default function TopDepartments({ config, departments }: TopDepartmentsProps) {
  const heading = config?.heading || 'Explore Leading Career Domains'
  const description = config?.description || "Uncover career paths tailored to your passion and goals. Explore industry-focused mentorship programs designed to guide you toward success."
  const buttonLabel = config?.buttonLabel || 'Browse All Courses'
  const buttonLink = config?.buttonLink || '/departments'
