...
export default function JoinUs(
  { instructor, title, startDateTime, whatsappLink, whatsappQrCode, slug, eventId }: JoinUsProps) {
      ...
      <div className="flex bg-secondary-100 w-[70%] gap-8 rounded-2xl max-xl:w-full  max-lg:flex-col  px-4 ">
        <div className='relative  flex items-end mx-auto'>
          <div className="relative w-[200px] md:w-[307px] aspect-[128/150]">
            <Image
              src={typeof instructor?.profileImage === 'object' ? (instructor?.profileImage?.url ?? '/nexusberryLogo.png') : '/nexusberryLogo.png'}
              alt={typeof instructor?.profileImage === 'object' ? (instructor?.profileImage?.alt ?? 'NexusBerry instructor') : 'NexusBerry instructor'}
              fill
      ...
      <div className="w-[30%] text-center space-y-4 bg-primary-100 rounded-2xl p-8 max-xl:w-full">
        ...
        <div className="flex flex-col gap-2 py-4 ">
          <div className="w-40 aspect-[1/1] max-xl:hidden mx-auto relative rounded-lg ring-1 ring-green-400">
            <Image fill
              src={typeof whatsappQrCode === "object" ? (whatsappQrCode?.url ?? '/placeholderImg.jpg') : '/placeholderImg.jpg'}
              alt={typeof whatsappQrCode === "object" ? (whatsappQrCode?.url ?? 'whatsappQrCode') : 'whatsappQrCode'}
              className="rounded-lg object-cover" sizes="160px" />
          </div>
          <Link
            href={`${whatsappLink ? whatsappLink : "#"}`}
            aria-label="whatsapp"
