...
const ResetPasswordForm: React.FC<any> = ({ token, isInitialSet }) => {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(errorData.message || 'An error occurred while setting your password.')
      }
    } catch (err) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{isInitialSet ? 'Set Your Password' : 'Reset Your Password'}</CardTitle>
        <CardDescription>
          {isInitialSet
            ? 'Enter your new password below to complete your registration.'
